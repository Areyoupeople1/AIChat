<template>
  <div>
    ...
    <el-table :data="tableData" :pagination="pagination">
      ...
      <el-table-column prop="diary_date" label="记录日期" width="120" />
      <el-table-column label="情绪评分">
        <template #default="scope">
          <el-rate :model-value="scope.row.mood_score" :max="10" disabled />
        </template>
      </el-table-column>
      <el-table-column label="生活指标" width="120">
        <template #default="scope">
          <div>
            <p>睡眠：{{ scope.row.sleep_quality }}/5</p>
            <p>压力：{{ scope.row.stress_level }}/5</p>
          </div>
        </template>
      </el-table-column>
      <el-table-column prop="emotion_triggers" label="情绪触发因素" width="120" />
      <el-table-column prop="diary_content" label="日记内容" width="250" />
    ...
    <el-dialog v-model="detailDiaVisable" title="情绪日志详情" width="800px" :close-on-click-modal="false">
        ...
        <div class="detail-section">
          ...
          <div class="ai-analysis-result">
            ...
            <div class="ai-risk-section">
              <h4>风险描述</h4>
              <div class="risk-content">{{ aiData.risDescription || '无' }}</div>
            </div>
      ...
      <template #footer>
        <el-button @click="detailDiaVisable = false">关闭</el-button>
      </template>
    </el-dialog>
  </div>
</template>
...
//详情弹窗
const detailDiaVisable = ref(false)
const currentDetail = ref(null)
const aiData = ref(null)
const viewSessionDetail = (row) => {
  currentDetail.value = row
  aiData.value = {}  // 情绪AI分析已移除
  detailDiaVisable.value = true
}
//删除
