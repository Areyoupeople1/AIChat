    ...
    <div class="content">
      <div class="diary-card">
        ...
        <div class="sub-title">
          <el-tag type="primary" size="large" class="category-tag">
            {{ articleDetail.category_name || '未分类' }}
          </el-tag>
          <div class="flex-box">
            <el-icon>
              <List/>
            </el-icon>
            <span>{{ formatDate(articleDetail.updated_at) }}</span>
          </div>
        </div>
        ...
        <div class="summary-content" v-if="articleDetail.summary">
          <p>{{ articleDetail.summary || '暂无摘要' }}</p>
        </div>
        ...
        <div class="flex-box" style="margin-top: 20px;">
          <div class="item">
            ...
            <span>{{ articleDetail.author_name || '系统管理员' }}</span>
          </div>
          <div class="item">
            <el-icon>
              <Platform/>
            </el-icon>
            <span>{{ articleDetail.read_count || 0 }}次阅读</span>
          </div>
        </div>
      </div>
      ...
      <div class="diary-card">
        ...
        <div class="tags-content" v-if="articleDetail.tagArray && articleDetail.tagArray.length > 0">
          ...
          <div class="tags-list">
            <el-tag v-for="tag in articleDetail.tagArray" :key="tag.id" size="small" type="info" effec="light"> 
              {{ tag }}
            </el-tag>
          </div>
        </div>
...
//   try {
//     const response = await getKnowledgeDetail(articleId)
